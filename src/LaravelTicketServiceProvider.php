...
use Binshops\LaravelTicket\Console\Htmlify;
use Binshops\LaravelTicket\Controllers\InstallController;
use Binshops\LaravelTicket\Controllers\NotificationsController;
...
use Binshops\LaravelTicket\ViewComposers\LaravelTicketComposer;
...
class LaravelTicketServiceProvider extends ServiceProvider
{
    ...
    public function boot()
    {
        ...
        if (empty($installer->inactiveMigrations()) && !$installer->inactiveSettings()) {
            ...
            LaravelTicketComposer::settings($u);
            ...
            LaravelTicketComposer::general();
            LaravelTicketComposer::codeMirror();
            LaravelTicketComposer::sharedAssets();
            LaravelTicketComposer::summerNotes();
            ...
            $this->loadTranslationsFrom(__DIR__.'/Translations', 'laravelticket');
            ...
            $this->loadViewsFrom($viewsDirectory, 'laravelticket');

            $this->publishes([$viewsDirectory => base_path('resources/views/vendor/laravelticket')], 'views');
            $this->publishes([__DIR__.'/Translations' => base_path('resources/lang/vendor/laravelticket')], 'lang');
            $this->publishes([__DIR__.'/Public' => public_path('vendor/laravelticket')], 'public');
...
//            $installer->publicAssets();
            ...
            $this->loadTranslationsFrom(__DIR__.'/Translations', 'laravelticket');
            $this->loadViewsFrom(__DIR__.'/Views/bootstrap3', 'laravelticket');
            $this->publishes([__DIR__.'/Migrations' => base_path('database/migrations')], 'db');
            ...
            Route::get('/tickets-install', [
                ...
                'uses'       => 'Binshops\LaravelTicket\Controllers\InstallController@index',
            ]);
            Route::post('/tickets-install', [
                'middleware' => $authMiddleware,
                'as'         => 'tickets.install.setup',
                'uses'       => 'Binshops\LaravelTicket\Controllers\InstallController@setup',
            ]);
            Route::get('/tickets-upgrade', [
                'middleware' => $authMiddleware,
                'as'         => 'tickets.install.upgrade',
                'uses'       => 'Binshops\LaravelTicket\Controllers\InstallController@upgrade',
            ]);
    ...
    public function register()
    {
        ...
        $this->app->singleton('command.binshops.laravelticket.htmlify', function ($app) {
            return new Htmlify();
        });
        $this->commands('command.binshops.laravelticket.htmlify');
    }
}
